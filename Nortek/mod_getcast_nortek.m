function [up,down,dataup,datadown] = mod_getcast_nortek(Meta_Data,min_depth,crit_depth)
...
if isfield(data,'Burst_Time')
    ...
    tdata=data.Burst_MatlabTimeStamp;
     timename='Burst_MatlabTimeStamp';
end
L=length(tdata);
...
hold on
for i=1:length(up)
        plot(dataup{i}.(timename),dataup{i}.Burst_Pressure,'r')
end
for i=1:length(down)
       plot(datadown{i}.(timename),datadown{i}.Burst_Pressure,'g')
end
...
switch answer1
    case 'yes'
        ...
        Meta_Data.adcpnbprofileup=numel(ADCPProfiles.up);
        Meta_Data.adcpnbprofiledown=numel(ADCPProfiles.down);
        Meta_Data.adcpmaxdepth=max(cellfun(@(x) max(x.Burst_Pressure),ADCPProfiles.dataup));
