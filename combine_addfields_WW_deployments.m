...
for ii = deployment_range
    ...
    %% make sure that there are no repeated values from telemetry
    [~,id,~] = unique(WWgrid.time);
    if length(id)<length(WWgrid.time)
        vars = fieldnames(WWgrid);
        for ii = 1:length(vars);
            [m,n] = size(WWgrid.(vars{ii}));
            if n>1
                WWgrid.(vars{ii}) = WWgrid.(vars{ii})(:,id);
            end
        end
    end
    
    %% Save it all and update indices
